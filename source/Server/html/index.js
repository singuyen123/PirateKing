...
// Object Constructors
function Fleet(name) { //ham doi
	this.name = name;
	//khai bao bien cac loai tau
	this.shipDetails = [{ "name": "carrier", "length": 5 },
						{ "name": "battleship", "length": 4 },
						{ "name": "cruiser", "length": 3 },
						{ "name": "destroyer", "length": 3 },
						{ "name": "frigate", "length": 2 }];
	this.numOfShips = this.shipDetails.length;//so luong tau
	this.ships = [];//tau
	this.currentShipSize = 0; //kich thuoc tau hien tai
	this.currentShip = 0; //tau hien tai
	...
	this.removeShip = function(pos) {
		this.numOfShips--;
		$(".text").text(output.sunk(this.name, this.ships[pos].name));//danh chim tau ten gi, ở vị trí nào 
...
function Ship(name){
	...
	this.getRidOf = function(pos) {
		this.hitPoints.splice(pos, 1);//thay thế vi tri đó bằng rỗng
	}
}

// Console obj
var output = {
	"welcome": " >Chào mừng bạn tới trò chơi BattleShip.  Sử dụng menu ở trên để bắt đầu.",
	"not": " >  Option hiện không khả dụng.",
	"player1": " > Bạn có muốn đặt tàu của riêng bạn hoặc có máy tính ngẫu nhiên làm điều đó cho bạn?",
	"self": " > Use the mouse and the Horizontal and Vertial buttons to place your ships on the bottom grid.",
	"overlap": " > Bạn không thể đặt chồng chéo tàu. Vui lòng thử lại.",
	"start": " > Sử dụng button trên board để bắn thuyền đội địch. Chúc may mắn!!",
...
//di chuyen chuot va chon vi trí đánh ở bản đồ của địch (topBoard)
		...
		$(square).off("click").on("click", function() {
			if(!($(this).hasClass("used"))) {
				...
				if (cpuFleet.ships.length == 0) {
 				...
 				} else setTimeout(bot.select, 100);
			} // end of if
		});
	},
}
//di chuyen chuot va chon vi trí đánh ở bản đồ của ta (bottomBoard)
var bottomBoard = {
	currentHits: [],
...
function gameSetup(t) {
	...
	$(".Two").addClass("random").removeClass("Two").text("Random");
